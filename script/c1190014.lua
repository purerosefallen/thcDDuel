...
function c1190014.filter(c)
	return c:IsFaceup() and c1190014.IsELF(c) and c:IsAbleToHand() and not c:IsCode(1190014)
end
function c1190014.tg1(e,tp,eg,ep,ev,re,r,rp,chk,chkc)
	if chkc then return chkc:IsOnField() and chkc:IsControler(tp) and c1190014.filter(chkc) end
	if chk==0 then return Duel.IsExistingTarget(c1190014.filter,tp,LOCATION_ONFIELD,0,1,nil) end
	Duel.Hint(HINT_SELECTMSG,tp,HINTMSG_RTOHAND)
	local g=Duel.SelectTarget(tp,c1190014.filter,tp,LOCATION_ONFIELD,0,1,1,nil)
